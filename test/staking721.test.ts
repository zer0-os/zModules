...
describe("Staking721", () => {
  ...
  describe("#viewRewardsInPool", () => {
    it("Allows a user to see the total rewards in a pool", async () => {
      const rewardsInPool = await staking721.viewRewardsInPool();
      expect(rewardsInPool).to.eq(await mockERC20.balanceOf(await staking721.getAddress()));
    });
  });
  ...
  describe("#stake | #stakeBulk", () => {
    ...
    it("Fails to stake when the caller is not the owner of the NFT", async () => {
      ...
      await expect(staking721.connect(staker).stake(nonStakedTokenId))
        .to.be.revertedWithCustomError(staking721, "InvalidOwner");
    });
  });
  ...
  describe("#viewRemainingTimeLock", () => {
    it("Allows the user to view the remaining time lock period for a stake", async () => {
      const remainingTimeLock = await staking721.viewRemainingTimeLock(tokenIdA);
      const timestamp = (await hre.ethers.provider.getBlock("latest"))?.timestamp;

      expect(remainingTimeLock).to.eq(BigInt(timestamp!) - stakedOrClaimedAtA);
    });
  });
  ...
  describe("#viewPendingRewards | #viewPendingRewardsBulk", () => {
    ...
    it("Can view pending rewards for multiple staked tokens", async () => {
      const pendingRewards = await staking721.viewPendingRewardsBulk([tokenIdB, tokenIdC]);
  ...
  describe("#claim | #claimBulk", () => {
    ...
    it("Can claim rewards on multiple staked tokens", async () => {
      ...
      await staking721.ownerOf(tokenIdB);
      await staking721.ownerOf(tokenIdC);
  ...
  describe("#unstake | #unstakeBulk", () => {
    ...
    it("Can unstake multiple tokens", async () => {
      ...
      await staking721.connect(staker).unstakeBulk([tokenIdB, tokenIdC]); // why invalid token id?
