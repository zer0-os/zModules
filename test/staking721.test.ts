...
describe.only("StakingERC721", () => {
  ...
  describe("#stake", () => {
    ...
    it("Fails to stake when the caller is not the owner of the NFT", async () => {
      ...
      await expect(
        ...
        .withArgs(stakingERC721.target, nonStakedTokenId);
    });
  });
  ...
  describe("Other configs", () => {
    ...
    it("More complex use case involving multiple stakes and stakers", async () => {
      ...
      let firstStakedAtB = origStakeAtB;
      ...
      let claimedAtB = firstUnstakedAtB;
      ...
      let secondStakedAtB = BigInt(await time.latest());
      ...
      let balanceAfterUnstakeTwoB = await localStakingERC721.balanceOf(stakerB.address);
